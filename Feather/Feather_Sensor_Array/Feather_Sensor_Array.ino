...
void loop() {
  ...
  ts.read_BH1745_color(RGB);
  ts.read_KMX62_mag(Mag); 
  BM1383_Pres = ts.read_BM1383_pressure();
  ...
  CO2ppmValue = ts.read_CO2_ppm();
  temp = ts.read_TMP116N_temp();
...
//WIFI message JSON object construction ------------------------------//
  ...
  PostData = PostData + "\"gyrox\":" + Gyro[0] + ",";
  PostData = PostData + "\"gyroy\":" + Gyro[1] + ",";
  PostData = PostData + "\"gyroz\":" + Gyro[2] + ",";
  PostData = PostData + "\"irArray\":[" + IRData + "],";
  PostData = PostData + "\"ambient\":" + vis_mag + ",";
  PostData = PostData + "\"uv\":" + uv_mag + ",";
