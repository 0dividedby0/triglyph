...
#define PressureOld    //BM1383GLV  //Use this definition with yellow stickered SHLD1 board. Triglyph has this one.
...
// ----- Demo Mode Definitions -----
#define CSVOutput
#define SensorSamplePeriod  2000 //in ms
...
//#define SCL_PIN 13  //A5       //Note that if you are using the I2C based sensors, you will need to download and
//#define SCL_PORT PORTB    //install the "SoftI2CMaster" as "Wire" does not support repeated start...
//#define SDA_PIN 12  //A4         //References:
//#define SDA_PORT PORTB    //  http://playground.arduino.cc/Main/SoftwareI2CLibrary
...
SlowSoftI2CMaster si = SlowSoftI2CMaster(70,71,false);
...
void setup()
{
  ...
  pinMode(51, OUTPUT);  //Added output for led
  pinMode(49, OUTPUT);  //Added output for led
  pinMode(47, OUTPUT);  //Added output for led
 ...
 //KMX62 Init Sequence
  ...
  si.i2c_start(KMX62_DeviceAddress);  //This needs the 8 bit address (7bit Device Address + RW bit... Read = 1, Write = 0)
  si.i2c_write(0x3A);
  si.i2c_write(0x5F);
  si.i2c_stop();
 #endif
 ...
 //----- Start Initialization for BM1383A Digital Pressure Sensor -----
  ...
  si.i2c_start(BM1383_DeviceAddress);  //This needs the 8 bit address (7bit Device Address + RW bit... Read = 1, Write = 0)
  si.i2c_write(0x12);
  si.i2c_write(0x01);
  si.i2c_stop();
...
//----- Start Initialization for BM1383 Digital Pressure Sensor -----
  ...
  si.i2c_start(RPR0521_DeviceAddress);  //This needs the 8 bit address (7bit Device Address + RW bit... Read = 1, Write = 0)
  si.i2c_write(0x41);
  si.i2c_write(0xE6);
  si.i2c_stop();
  ...
  si.i2c_start(BH1745_DeviceAddress);  //This needs the 8 bit address (7bit Device Address + RW bit... Read = 1, Write = 0)
  si.i2c_write(0x61);
  si.i2c_write(0x03);
  si.i2c_stop();
  ...
  si.i2c_start(KX122_DeviceAddress);  //This needs the 8 bit address (7bit Device Address + RW bit... Read = 1, Write = 0)
  si.i2c_write(0x18);
  si.i2c_write(0x40);
  si.i2c_stop();
  ...
  si.i2c_start(KXG03_DeviceAddress);  //This needs the 8 bit address (7bit Device Address + RW bit... Read = 1, Write = 0)
  si.i2c_write(0x43);
  si.i2c_write(0x00);
  si.i2c_stop();
...
#ifdef MagField
  si.i2c_start(BM1422_DeviceAddress);  //This needs the 8 bit address (7bit Device Address + RW bit... Read = 1, Write = 0)
  si.i2c_write(0x1B);
  si.i2c_write(0xC0);
  si.i2c_stop();
...
*/
  ...
  Serial.write(0x0D);  //Print Carrage Return
  #endif
//Begin reading IR camera data
//COMMENTED , DONT WANT TO PRINT IR CAMERA PIXELS
...
//COMMENTED OUT IGNORE PRINTING IR PIXELS
      //read all the pixels
...
//
  ...
  //Arduino Due uses an 3.3V ADC Reference Voltage.
  //The ADC on the Due is set to 10 bits of resolution by default.
  //The voltage per bit is therefore calculated to be 3.3V/1024bits.
  
  float voltsPerBitTemp = 3.3/1024;
  int sensorValueTemp = analogRead(ADCpin_AnalogTemp);
  
  //Calculations for Analog Temp Sensor - BDE0600G
  // Temperature Sensitivity = -10.68mV/degC = -0.01068V/degC
  // Known Temp/Voltage Point = 1.753V @ 30DegC

  float sensitivityTemp = -0.01068;
  int knownTemp = 30;
  float knownVoltsTemp = 1.753;
  
  float sensorConvertTemp = (float)sensorValueTemp*voltsPerBitTemp;
  sensorConvertTemp = (sensorConvertTemp-knownVoltsTemp)/(sensitivityTemp)+knownTemp;

//Output the read values as you desire.
...
//  Serial.write(0x0D); //Print Carrage Return
  ...
  Serial.print(sensorConvertTemp);
  Serial.write(" degC");
  Serial.write(0x0A); //Print Line Feed
  Serial.write(0x0D); //Print Carrage Return
  lineClear += 1;
  ...
  float voltsPerBitUV = 3.3/1024;
  int sensorValueUV = analogRead(ADCpin_AnalogUV);

  //Calculations for UV Sensor - ML8511
  //Sensitivity = 0.129V/(mW/cm2)
  //Known Point = 2.2V @ 10mW/cm2

  float sensitivityUV = 0.129;
  int knownIntensityUV = 10;
  float knownVoltsUV = 2.2;

  float sensorConvertUV = (float)sensorValueUV*voltsPerBitUV;
  sensorConvertUV = (sensorConvertUV-knownVoltsUV)/(sensitivityUV)+knownIntensityUV;

  //Output to the terminal in the desired format.
  //Uncomment this for Raw Values
//  Serial.write("ADC Raw Value Return = ");
...
//  Serial.write(0x0D); //Print Carrage Return
  ...
  //Due                 20 (SDA), 21 (SCL), SDA1, SCL1 [THIS is the platform we are using]
  ...
  si.i2c_start(KMX62_DeviceAddress);
  si.i2c_write(0x0A);
  si.i2c_rep_start(KMX62_DeviceAddress | 1);  // Or-ed with "1" for read bit
  MEMS_Accel_Xout_lowByte = si.i2c_read(false);
  MEMS_Accel_Xout_highByte = si.i2c_read(false);
  MEMS_Accel_Yout_lowByte = si.i2c_read(false);
  MEMS_Accel_Yout_highByte = si.i2c_read(false);
  MEMS_Accel_Zout_lowByte = si.i2c_read(false);
  MEMS_Accel_Zout_highByte = si.i2c_read(true);
  si.i2c_stop();
  ...
  */
  ...
  si.i2c_write(0x10);
  si.i2c_rep_start(KMX62_DeviceAddress | 1);  // Or-ed with "1" for read bit
  MEMS_Mag_Xout_lowByte = si.i2c_read(false);
  MEMS_Mag_Xout_highByte = si.i2c_read(false);
  MEMS_Mag_Yout_lowByte = si.i2c_read(false);
  MEMS_Mag_Yout_highByte = si.i2c_read(false);
  MEMS_Mag_Zout_lowByte = si.i2c_read(false);
  MEMS_Mag_Zout_highByte = si.i2c_read(true);
  si.i2c_stop();
  ...
  */
  ...
  si.i2c_start(BM1383_DeviceAddress);
  si.i2c_write(0x1A);
  si.i2c_rep_start(BM1383_DeviceAddress | 1);  // Or-ed with "1" for read bit
  //For new version of Pressure Sensor (BM1383AGLV)
  BM1383_Pres_highByte = si.i2c_read(false);
  BM1383_Pres_lowByte = si.i2c_read(false);
  BM1383_Pres_leastByte = si.i2c_read(false);
  BM1383_Temp_highByte = si.i2c_read(false);
  BM1383_Temp_lowByte = si.i2c_read(true);
/*
...
*/
  ...
  //For Old version of PRessure Sensor (BM1383GLV)
  BM1383_Temp_highByte = si.i2c_read(false);
  BM1383_Temp_lowByte = si.i2c_read(false);
  BM1383_Pres_highByte = si.i2c_read(false);
  BM1383_Pres_lowByte = si.i2c_read(false);
  BM1383_Pres_leastByte = si.i2c_read(true);
  ...
  si.i2c_start(RPR0521_DeviceAddress);
  si.i2c_write(0x42);
  si.i2c_rep_start(RPR0521_DeviceAddress | 1);  // Or-ed with "1" for read bit
  RPR0521_PS_LB = si.i2c_read(false);
  RPR0521_PS_HB = si.i2c_read(false);
  RPR0521_PS_LB = si.i2c_read(false);
  RPR0521_PS_HB = si.i2c_read(false);
  RPR0521_ALS_D0_LB = si.i2c_read(false);
  RPR0521_ALS_D0_HB = si.i2c_read(false);
  RPR0521_ALS_D1_LB = si.i2c_read(false);
  RPR0521_ALS_D1_HB = si.i2c_read(true);
  si.i2c_stop();
  ...
  si.i2c_start(BH1745_DeviceAddress);
  si.i2c_write(0x50);
  si.i2c_rep_start(BH1745_DeviceAddress | 1);  // Or-ed with "1" for read bit
  BH1745_RED_LB = si.i2c_read(false);
  BH1745_RED_HB = si.i2c_read(false);
  BH1745_GRN_LB = si.i2c_read(false);
  BH1745_GRN_HB = si.i2c_read(false);
  BH1745_BLU_LB = si.i2c_read(false);
  BH1745_BLU_HB = si.i2c_read(true);
  si.i2c_stop();
  ...
  if(BH1745_GRN_OUT > 1000){
    digitalWrite(51, HIGH);
  }
  else{
    digitalWrite(51, LOW);
  }
  if(BH1745_BLU_OUT > 1000){
    digitalWrite(49, HIGH);
  }
  else{
    digitalWrite(49, LOW);
  }
  if(BH1745_RED_OUT > 300){
...
#ifdef KX122
  si.i2c_start(KX122_DeviceAddress);
  si.i2c_write(0x06);
  si.i2c_rep_start(KX122_DeviceAddress | 1);  // Or-ed with "1" for read bit
  KX122_Accel_X_LB = si.i2c_read(false);
  KX122_Accel_X_HB = si.i2c_read(false);
  KX122_Accel_Y_LB = si.i2c_read(false);
  KX122_Accel_Y_HB = si.i2c_read(false);
  KX122_Accel_Z_LB = si.i2c_read(false);
  KX122_Accel_Z_HB = si.i2c_read(true);
  si.i2c_stop();
...
if (t == 1){
  int i = 11;
  while(--i){
  si.i2c_start(KXG03_DeviceAddress);
  si.i2c_write(0x02);
  si.i2c_rep_start(KXG03_DeviceAddress | 1);
  KXG03_Gyro_X_LB = si.i2c_read(false);
  KXG03_Gyro_X_HB = si.i2c_read(false);
  KXG03_Gyro_Y_LB = si.i2c_read(false);
  KXG03_Gyro_Y_HB = si.i2c_read(false);
  KXG03_Gyro_Z_LB = si.i2c_read(false);
  KXG03_Gyro_Z_HB = si.i2c_read(true);
  si.i2c_stop();
...
#ifdef MagField
  si.i2c_start(BM1422_DeviceAddress);
  si.i2c_write(0x10);
  si.i2c_rep_start(BM1422_DeviceAddress | 1);
  BM1422_Mag_X_LB = si.i2c_read(false);
  BM1422_Mag_X_HB = si.i2c_read(false);
  BM1422_Mag_Y_LB = si.i2c_read(false);
  BM1422_Mag_Y_HB = si.i2c_read(false);
  BM1422_Mag_Z_LB = si.i2c_read(false);
  BM1422_Mag_Z_HB = si.i2c_read(true);
  si.i2c_stop();
