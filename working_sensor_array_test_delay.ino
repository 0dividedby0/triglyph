...
// ----- Demo Mode Definitions -----
#define CSVOutput
#define SensorSamplePeriod  1000 //in ms
...
SlowSoftWire ssw = SlowSoftWire(70,71,false);
...
void setup()
{
  Wire1.begin();        // start I2C functionality
...
//CO2 Initialization code must be called every loop so it is not neccessary to intitate separately from the read loop
//End CO2 Sensor Init
...
void loop()
{
  ...
  */
  ...
  sensorConvert = (float)sensorValue *VoltsPerBit;
  sensorConvert = (sensorConvert - 1.753)/(-0.01068)+30;
  
  #ifndef CSVOutput
  Serial.write("BDE0600G Temp = ");
  Serial.print(sensorConvert-7);
  Serial.write(" degC");
  Serial.write(0x0A); //Print Line Feed
  Serial.write(0x0D); //Print Carrage Return
  lineClear += 1;
...
if (t == 1){
  ...
  */
  ...
  #ifdef CO2
  //CO2 Sensor read ppm data function
  // intialize I2C using wire on SDA and SCL not connected to ROM board
  ssw.beginTransmission(CO2_ADDR);
  ssw.write(0x04); //write function code
  ssw.write(0x13); //write register address(MSB)
  ssw.write(0x8B); //write register address(LSB)
  ssw.write(0x00); //write # of registers (MSB)
  ssw.write(0x01); //write # of registers (LSB)
  ssw.endTransmission();
  
  delay(10);
  
  ssw.requestFrom(CO2_ADDR,4); //request 4bytes from CO2
  data[0] = ssw.read();  //not all reads from the CO2 sensor are relevant
  data[1] = ssw.read();  
  data[2] = ssw.read();  //data used for ppm calculation
  data[3] = ssw.read();  //data used for ppm calculation
  CO2ppmValue = ((data[2] & 0x3F) << 8) | data[3];  //ppm calculation
